...
localizeKeybindings = () ->
  ...
  pref.find('.section-heading.icon.icon-keyboard:contains("Keybindings")').html('快捷键设置')

  temp = pref.find('.keybinding-panel div.text.native-key-bindings').children()
  temp.eq(1).html('您能覆盖这些快捷键设置。只要按下&nbsp;')
  temp.eq(3).html('图示复制，并粘贴到您的&nbsp;')
  temp.eq(4).html('快捷键设置文件&nbsp;')
  temp.eq(4).after('<span>中修改即可。</span>') if temp.length == 5

  pref.find('table th.keystroke').html('按键输入')
  pref.find('table th.command').html('执行指令')
...
localizePackages = () ->
  ...
  temp.contents().first()[0].textContent='已安裝的扩展插件' if temp.length > 0

  temp = pref.find('.sub-section-heading.icon.icon-package:contains("Deprecated Packages")')
  temp.contents().first()[0].textContent='已过期的扩展插件' if temp.length > 0

  pref.find('section.sub-section.deprecated-packages p').html('Atom 不会载入过期的扩展插件。这些扩展插件可能有提供更新。')

  temp = pref.find('.sub-section-heading.icon.icon-package:contains("Community Packages")')
  temp.contents().first()[0].textContent='来自社区的扩展插件' if temp.length > 0

  temp = pref.find('.sub-section-heading.icon.icon-package:contains("Core Packages")')
  temp.contents().first()[0].textContent='核心扩展插件' if temp.length > 0

  temp = pref.find('.sub-section-heading.icon.icon-package:contains("Development Packages")')
  temp.contents().first()[0].textContent='开发中的扩展插件' if temp.length > 0

  si = setInterval((->
    temp = pref.find('.icon.icon-dashboard.native-key-bindings:contains("This package added ")')
    if temp.length != 0
      temp.html temp.html().replace('This package added ', '这个插件增加了 ').replace(' to startup time.', ' 的启动时间。')
  ), 0)
...
localizeUpdates = () ->
  ...
  pref.find('.alert-info.icon.icon-heart').html('您所有的扩展插件都已经是最新版本了。')
...
localizeInstall = () ->
  ...
  si = setInterval((->
    ...
    pref.find('.section-heading.icon.icon-star:contains("Featured Packages")').html('热门扩展插件')
...
localizeButtons = () ->
  ...
  si = setInterval((->
    pref.find('button:contains("Packages")').html '扩展插件'
    ...
    pref.find('button:contains("View on Atom.io")').html '在 Atom.io 查看此插件'
